...
CAT = FV(2,:);
DN = strings (Bize (DVN)) ; DN(cellfun(@(x)ischar(x)||isstring(x),DVN)) = string(DVN(cellfun(@(x)ischar(x)||isstring(x),DVN)));
DV = stringe (Bize (DCN) ); DV(cellfun(@(x)ischar(x)||isstring(x),DCN)) = string(DCN(cellfun(@(x)ischar(x)||isstring(x),DCN)));

DIVe_Mod_Data = struct; 

for k=1:nM
c= CAT (k);
    DIVe_Mod_Data.(c+" a") = join(FN(:,k),newline);
    DIVe_Mod_Data.(c+" b") = join(FV(:,k),newline) ;
    DIVe_Mod_Data.(c+"_c") = join(DV(DV(:,k)~="",k),newline);
    DIVe_Mod_Data.(c+" d") = join(DN(DN(:,k)~="",k),newline);
end
