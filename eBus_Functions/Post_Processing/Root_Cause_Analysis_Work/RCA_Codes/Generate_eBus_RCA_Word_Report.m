...
localAddHeading(selection, '10.5.3 Worked Segment Sequence Illustration', 3);
if height(reportData.SegmentSummary) > 0
    selection.TypeText(['The table below shows the first few segments from the current RCA run. ', ...
        'It is intended as a worked example of how the segmentation logic translates the continuous drive cycle into reviewable operating blocks with motion class, grade class, auxiliary class, and dominant gear.']);
    selection.TypeParagraph;
    exampleTable = localBuildSegmentationExampleTable(reportData.SegmentSummary);
    localAddWordTable(doc, selection, 'Worked example of segment sequence from the current RCA run', ...
        exampleTable.Properties.VariableNames, localTableToCellRows(exampleTable));
else
    selection.TypeText('[Insert segment-sequence illustration using representative segment rows from the current run.]');
    selection.TypeParagraph;
end
end
...
localAddHeading(selection, '10.6.3 Worked Example of Bad-Segment Selection', 3);
if height(reportData.BadSegmentTable) > 0
    selection.TypeText(['The table below shows representative bad segments selected by the current RCA run. ', ...
        'This is the direct shortlist handed forward into the root-cause ranking step, where each segment receives a ranked contributor list and supporting evidence signals.']);
    selection.TypeParagraph;
    exampleTable = localBuildBadSegmentExampleTable(reportData.BadSegmentTable);
    localAddWordTable(doc, selection, 'Worked example of bad-segment shortlist from the current RCA run', ...
        exampleTable.Properties.VariableNames, localTableToCellRows(exampleTable));
else
